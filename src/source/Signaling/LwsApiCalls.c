...
INT32 lwsWssCallbackRoutine(struct lws* wsi, enum lws_callback_reasons reason, PVOID user, PVOID pDataIn, size_t dataSize)
{
    ...
    lws_set_log_level(LLL_NOTICE | LLL_INFO | LLL_WARN | LLL_ERR | LLL_DEBUG, NULL);
...
STATUS lwsCompleteSync(PLwsCallInfo pCallInfo)
{
    ...
    *pHostEnd = '\0';
    ...
    locked = TRUE;
    printf("lwsCompleteSync(): call lws_client_connect_via_info()\n");
...
STATUS connectSignalingChannelLws(PSignalingClient pSignalingClient, UINT64 time)
{
    ...
    printf("connectSignalingChannelLws(): URL= %s\n", url);
    ...
    // Wait for ready
    printf("connectSignalingChannelLws(): Waiting for connection");
